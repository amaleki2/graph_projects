...
from case_studies.sdf.util_sdf import (cells_to_edges, vertices_to_proximity, vertex_to_proximity_kdtree,
                                       compute_edge_features, add_reversed_edges)
...
def get_sdf_data_list(rnd_idx, data_folder, edge_method='edge', edge_params=None, n_jobs=1,
                      no_global=False, no_edge=False, with_normals=False, include_reverse_edges=False):
    def func(idx):
        mesh_file = os.path.join(data_folder, "volume_mesh_%d.vtk" % idx)
...
def get_sdf_data_loader_3d(n_objects, processed_cad_data_folder, volume_mesh_data_folder=None,
                           batch_size=1, eval_frac=0.2, no_edge=False, with_normals=False, n_jobs=1,
                           n_volume_points=5000, edge_method='edge', edge_params=None, no_global=False,
                           data_parallel=False, shuffle=False, include_reverse_edges=False, train_idx=None):
    ...
    train_graph_data_list = get_sdf_data_list(train_idx, volume_mesh_data_folder, edge_method=edge_method,
                                              edge_params=edge_params, include_reverse_edges=include_reverse_edges,
                                              no_global=no_global, no_edge=no_edge, with_normals=with_normals,
                                              n_jobs=n_jobs)

    print("computing test set")
    test_graph_data_list  = get_sdf_data_list(test_idx, volume_mesh_data_folder, edge_method=edge_method,
                                              edge_params=edge_params, include_reverse_edges=include_reverse_edges,
                                              no_global=no_global, no_edge=no_edge, with_normals=with_normals,
                                              n_jobs=n_jobs)
