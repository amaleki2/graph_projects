from case_studies.mesh_morphing import get_mesh_morphing_data_loader, train_mesh_morphing
from src import (GATUNet, GCNUNet, EncodeProcessDecode, regular_loss, graph_loss, parse_arguments, chamfer_loss)
...
if network_name == "gat":
    ...
    loss_funcs = [chamfer_loss]
else:
    raise(ValueError("model name %s is not recognized" %network_name))

# load data
train_data, test_data = get_mesh_morphing_data_loader(n_objects, data_folder, batch_size, eval_frac=eval_frac,
                                                      edge_method=edge_method, edge_params=edge_params,
                                                      no_global=no_global)
# train
train_mesh_morphing(model, train_data, test_data, loss_funcs, n_epochs=n_epochs, print_every=print_every,
